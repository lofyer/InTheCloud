...
此种算法计算复杂度不高，对中间值缺失不敏感，但可能会产生over-fitting的问题？


朴素贝叶斯
~~~~~~~~~~

朴素贝叶斯模型是基于独立概率统计的，思想大概可以这么说：

.. code::

    总共7个石子在A、B两个桶中，A桶中有2黑2白，B桶中有2黑1白。已知条件为石子来自B桶，那么它是白色石子的概率可表示为：

        P(white|B)=P(white*B)/P(B)

    其中，P(B)为从B桶中取出石子的概率，P(white*B)为从B桶中取出白色石子的概率，P(B)为从B桶中取出石子的概率，即为(1/7)/(3/7)=1/3。
